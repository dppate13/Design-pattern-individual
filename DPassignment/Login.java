...
class Login extends JDialog {
	...
	private void loginButton_actionPerformed(ActionEvent e) {
		...
		try {
			...
			{
				UserType = UserInfoItem.USER_TYPE.Buyer; // 0 = buyer
				file = new BufferedReader(new FileReader("BuyerInfo.txt"));
			} else// seller
			{
				UserType = UserInfoItem.USER_TYPE.Seller; // 1 = seller
	...
	/* after login is completed we get the UserName of the login interface */
	String GetUserName() {
		return UserBox;
	}

	/* after login is completed we get the userType of the login interface */
